...
public function createIncident($reporter_name, $reporter_contact, $description, $lat, $lng, $severity, $media_path = null) {
    $stmt = $this->pdo->prepare("INSERT INTO incident_reports 
        (reporter_name, reporter_contact, description, lat, lng, severity, media) 
        VALUES (?, ?, ?, ?, ?, ?, ?)");
        
    $stmt->execute([$reporter_name, $reporter_contact, $description, $lat, $lng, $severity, $media_path]);
